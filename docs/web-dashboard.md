...
- **통합 관리 대시보드** : 데스크탑 에이전트와 연동되어 사용자의 일정 및 작업 정보, 자신의 작업 세션에 대한 피드백 데이터를 한눈에 조회하고 관리하는 웹 기반 대시보드로서의 역할을 목표로 합니다.

- **실제 데이터 기반의 피드백 제공** : 사용자의 세션 데이터를 활용하여, 사용자의 작업 패턴이나 경향성을 파악하여 이에 맞는 개인화된 형태의 피드백을 제공하는 것을 목표로 합니다. 

### 1.2 주요 기능
- **일정 정보 종합 관리** : 사용자가 자신의 스케줄을 직접 CRUD(생성, 조회, 수정, 삭제) 할 수 있는 인터페이스를 제공합니다.

- **작업 유형별 관리 및 환경 설정** : 특정 작업 유형에 따라 강제 실행될 프로그램을 사용자가 직접 지정하고 관리할 수 있습니다.

- **Gemini 2.5 기반 활동 피드백** : 사용자의 세션 데이터를 분석하여 AI가 생성한 활동 요약 및 맞춤형 개선 피드백을 제공합니다.
...
| Method | Endpoint | 설명 | 로그인 권한 |
|:---:|:---|:---|:---:|
| `GET` | `/insight` | Gemini 2.5 기반 피드백 제공 | ✅ |
...
* **상세**: `useTasks` 훅을 사용하여 사용자가 정의한 작업 환경 설정을 관리합니다.
    * **강제 실행 설정**: 특정 작업 시작 시 자동으로 실행될 프로그램 경로 등 지정
    * **환경 제어**: 해당 작업 중 강제 실행할 애플리케이션 리스트 구성
```mermaid
graph LR
    A[사용자] --> B[작업 유형 추가 및 선택]
    B --> C[강제 실행 프로그램 등록]
    C --> D[작업 유형별 프로그램 정보 DB 저장]
    D --> E[에이전트 실행 정책 반영]
```
...
### 5.5 Feedback (피드백)
* **기능**: **Gemini 2.5** 모델을 활용하여 사용자의 활동을 분석하고 맞춤형 개선안을 제시합니다.
* **상세**:
...
```mermaid
graph LR
    A[활동 데이터] --> B[Gemini 2.5 AI 기반 분석]
    B --> C{피드백 생성}
    C --> D[종합 피드백 제공]
    C --> E[집중도 피드백 제공]
    C --> F[피로도 피드백 제공]
