...
async def analyze_session_insight(
    ...
    if is_data_insufficient:
        ...
        1. **내부 목표 대조 및 은닉**: 
           ...
           - 사용자 응답에는 '.exe'와 같은 파일명이나 허용 리스트 자체를 절대 명시적으로 밝히지 마세요.
        ...
        3. **영문 배지**: 성과에 따라 'DEEP WORK', 'GOAL ACHIEVED', 'DISTRACTED', 'RECOVERY NEEDED' 등 상황에 맞는 영문 배지를 부여하세요.

        4. **카드 구성**:
           - '📊 요약': 설정한 작업 목표와 실제 활동 사이의 정렬도(Alignment)를 중심으로 요약.
           - '✅ 양호한 점': 작업의 본질에 집중하여 생산성을 높였던 구간에 대한 구체적인 칭찬.
           - '🚀 개선이 필요한 점': 내부 기준에서 벗어난 활동이 발생한 시점과 그로 인한 집중력 손실을 지적하고, 환경 개선 제안.

        5. **텍스트 강조**: 특정 앱 이름, 시간대, 점유율 수치는 반드시 **볼드체**(**내용**)로 표기하세요.

        6. 모든 응답은 InsightResponse JSON 구조를 엄격히 따르며, 한국어로 친절하고 전문적인 코치로서의 톤을 유지하세요.
        """
